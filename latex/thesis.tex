...
\subsection{Point Inversion}
	Let $\gamma(t) \subset \mathbb{R}^3$ a continuous curve, $t \in [0,1]$, and let $Q \in \mathbb{R}^3$ a point.
	We want to find
	...
	Although in CoolingGen, we often cannot make the assumption of differentiability or convexity of $\phi$. We therefore employ a pragmatic scheme and assume piecewise differentiability and piecewise convexity of $\phi$ and run Newton's method on a small subinterval $[l,u] \subset [0,1]$.
	...
	We let $l = m - i/h$ and $u = m + i/h$. Next, we can use Newton's method with the start value $m$ to find
