...
\section{Chambers}\label{sec:chambers}
	...
	First, we apply the stream surface coordinate transformation presented in Section \ref{sec:mrtheta} to a fixed number $N$ of $v$-isoparametric curves of the blade to calculate the profiles $B^{(v_i)}_\textrm{stream}(u)$ for $i \in \{1,...,N\}$. These profiles are then offset by an input parameter. The profile is automatically trimmed adequately and fillets are added, the radius of which are also declared as an input. We refer to the resulting profile as \emph{shrunk profile} and declare it $S^{(v_i)}_\textrm{stream}(u)$. This process is depicted in Figure \ref{fig:profileshrinking}. The subdivision into leading edge, pressure side, suction side and trailing edge is kept by using the fillet boundaries as transition points.
	...
	Next, the blade is partitioned at the intersection points $p_j^+$ and $p_j^-$ for all $j \in \{1,...,N_C-1\}$. For each such partitioning, we are left with four chamber-delimiting curves per chamber profile. This procedure is exemplified in Figure \ref{fig:subdivision_chamber}. If there is only one chamber, the old partitioning scheme (leading edge, suction side, pressure side and trailing edge) is kept. In case of multiple chambers, the leading and trailing chambers have to be treated differently, because then each of them are delimited by only one wall. However, we can use the leading edge partition or trailing edge partition in place of the respective wall.
	...
	The leading and trailing partition offsets are linearly interpolated between suction and pressure side offsets to guarantee continuity and differentiability of the chamber profile at the leading and trailing edge transitions. After offsetting, the chamber-delimiting curve end points do not necessarily intersect. Therefore, we connect the curve ends either by curve-curve intersection, ray-curve intersection or ray-ray intersection, depending on which one is applicable. The rays are in this case defined by $R_{A,B}$, where $A$ represent the curve end points and $B$ represent the gradient at the curve boundary points. These rays can be understood as the linear continuation of the chamber-delimiting curves (see \ref{fig:linear_continuation_curve}).
...
\section{Film Cooling Holes}
	A film cooling hole is a hole that transfers the coolant from the inside of the channel to the outside of the blade. An array of these holes provides a protective fluid layer that restrains heat transfer to the blade.
	...
	In CoolingGen, the geometric shape of a film cooling hole is described using two delimiting curves and a transitional $v$-parameter. CoolingGen currently supports two types of delimiting parametric curves, but can be extended to provide a multitude of such curves. These types of curves are the circular curve
		$$ f_\textrm{circular}(t) =
			\begin{pmatrix}
				r \cos 2\pi t \\
				r \sin 2\pi t
			\end{pmatrix}
		$$
	for a given radius $r$ and the more complex fan shaped curve
	...
	To construct a surface from a delimiting curve $f$, we canonically embed the two-dimensional curves into three-dimensional space using the map
		$$ E: \mathbb{R}^2 \mapsto \mathbb{R}^3, \quad (x, y) \rightarrow (x, y, 0). $$
	We also require a line segment $L_{A,B}$ between two points $A, B \in \mathbb{R}^3$.
	The curve $E(f)$ lies in the $(x,y)$-plane that has the normal vector $N_{(x,y)} = (0, 0, 1)$. We construct the rotation matrix $R \in \mathbb{R}^{3 \times 3}$ that rotates $N_{(x,y)}$ onto the direction $(B-A)$ of the line segment $L_{A,B}$ by finding the angle between $(B-A)$ and $N_{(x,y)}$. The rotation axis is given by the cross product $(B-A) \times N_{(x,y)}$. Using the rotation matrix $R$, we can rotate $E(f)$ to find $RE(f)$. We can translate $RE(f)$ to the start and end point of $L_{A,B}$ to find the curves
		$$ f_A := A+RE(f) \quad\textrm{and}\quad f_B = B+RE(f). $$
	For each $u, v \in [0,1]$, we can linearly interpolate between $f_A(u)$ and $f_B(u)$ to find the surface 
		$$F(u,v) = (1-v)\,f_A(u) + v\,f_B(u).$$
	We call the surface $F(u,v)$ the extrusion of $f$ along $L_{A,B}$.

	Setting a value $v_\textrm{transition} \in [0,1]$, we can combine two surfaces extrusion surfaces of different curve types along the same line segment. For this purpose, let $F_1(u,v)$ be the extrusion of $f_1(t)$ along $L_{A,B}$ and let $F_2(u,v)$ be the extrusion of $f_2$ along $L_{A,B}$. We can now write the combined surface of $F_1$ and $F_2$ along $L_{A,B}$ as
		$$
			C^{(v_\textrm{trans})}(u,v) = 
				\begin{cases}
					F_1(u,v) 																										& \textrm{ if } v \leq v_\textrm{trans} \\
					\frac{v-v_\textrm{trans}}{1-v_\textrm{trans}} F_1(u,v_\textrm{trans}) + \frac{v}{1-v_\textrm{trans}} F_2(u,1)	& \textrm{ else.} 
				\end{cases}
		$$

	The surface $C^{(v_\textrm{trans})}$ is a representation of a film cooling hole. There are currently two supported types of hole surfaces in CoolingGen. The first type is the cylindrical hole, where $f_1 = f_2 = f_\textrm{circular}$. The second type of film cooling hole is $f_1 = f_\textrm{circular}$ and $f_2 = f_\textrm{fan shaped}$. Both of these types can be reviewed in Figure \ref{fig:filmholes}.
