...
\begin{proof}
	...
	\end{align*}
	which is equal to $C_P(t)$ for degree $n+1$ and some arbitrary value of $t \in [0, 1]$.
\end{proof}

A visual representation of algorithm \ref{alg:decasteljaualgo} yields a triangular scheme. To compute one point on a Beziér curve $C_P$ with degree $n$, one has to perform $n^2-n$ additions and just as many scalar multiplications.
