...
\subsection{Intersecting a Surface and a Plane}
	In this section, we present an algorithm that intersects a plane and a surface in $\mathbb{R}^3$, which is hugely motivated by \cite{Barnhill1990}. The intersection generally takes the form of a surface, a curve, a point or any union of the aforementioned. In the case of CoolingGen, we only care about the case that the surface-plane intersection is a single curve. This assumption is implicitly made in many of the statements involving the intersection algorithm. First, we need to introduce some basic notions.
	...
	\begin{figure}[H]
		\centering
		\includesvg[width=\textwidth]{../python/surfacePlaneIntersection1}
	...
	\begin{figure}[H]
		\centering
		\includesvg[width=.8\textwidth]{../python/surfacePlaneIntersection2}
...
\section{The S2 Stream Surface and Stream Surface Coordinates}\label{sec:mrtheta}
	...
	To design the blades for turbomachinery, BladeGen and CoolingGen make use of a certain $S_2$ surface, the so called $S_{2,m}$ surface. This surface lies midway between two blades \cite{Wu1952}. Each stream line described by $S_{2,m}$ can then be written as $S_{2,m}(\cdot, v)$. By intersecting the cylindrically transformed $u$-isoparametric curves $Z(B(u,\cdot))$ of the blade with each stream line $S_{2,m}(\cdot, v)$, we can construct a curve $B^{(v)}_\textrm{stream}(u)$ from the intersection points $P_{u,v}$, given by
