...
\subsection{Definition}
	...
	In a Beziér curve of a given degree, the control points completely determine the shape of the curve. This behavior can be observed in Figure \ref{fig:bezierdifferentdegrees}. However, changing one control point affects the whole curve, since for $n$ control points the degree of the Beziér curve is always $n-1$. This behavior also becomes apparent using de Casteljau's algorithm for the computation of points on the Beziér curve.
...
\subsection{Properties}
	Other than being remarkably intuitive, Beziér curves have a lot of covenient properties. In computer-aided design software, most graphical user interfaces rely on the principle of letting the user interactively drag and drop the control points with a mouse, granting them control over the shape of the whole Beziér curve. The following theorems further illustrate why this is a good concept.
	...
	Theorem \ref{thm:bezierendpoints} guarantees the control over the end points of the the curve, whereas Theorem \ref{thm:bezierconvexhull} ensures that the Beziér curve lies close to the control points. Theorem \ref{thm:bezieraffinetransform} shows that transformations such as rotations and projections can be applied directly to the control points instead of the points on the curve.

	Simple as their appearance may be, Beziér curves fall short of representing some of the most common geometric shapes. Given a finite number of control points, we can never make $B_P(t)$ a circular arc, although a circle has a very simple parametric form. One of their greatest perks, the ability to describe a shape with a low number of of control points, is simultaneously one of their greatest shortcoming. This is most likely the reason why Beziér curves are not the state of the art in technical engineering applications. However, Beziér curves certainly do provide an intuition for Non-Uniform Rational B-Splines or NURBS, which is their prevailing counterpart.
...
\subsection{Properties}
	...
	For a given degree $p$, B-spline and NURBS curves can have any number $n > p+1$ of control points. This makes them a much more flexible tool than their Beziér counterpart. Yet Beziér, B-splines and NURBS curves still share a lot of nice properties, which makes them, once implemented, easy to use.

\section{Common Methods on Curves and Surfaces}
	In this section, the most important tools of CoolingGen are presented. However simple the modification of NURBS objects through their control points may be, common and useful operations such as the calculation of intersection points, the projection of an arbitrary point in space onto a curve and finding useful offset curves and fillets are major challenges. (Insert SIAM journal citation about how much cost bad intersection algorithms generated).
...
\subsection{Point Inversion}
	...
	\begin{figure}[H]
		...
		\caption{Point inversion. Each blue point on the orange curve is the projection of the point connected to it with a dotted line.}
	...
	To find $[l,u]$, we sample the curve $\gamma$ at a set of points $\{t_0, t_1, ..., t_n\}$ with $t_i = \frac{i}{n}$ and seek
	...
	If the target distance is not equal to zero, the algorithm can be used to project points onto the curve, as can be seen in \ref{fig:pointinversion}. If the curve is furthermore not injective, the algorithm still proves effective to find the distance $D(Q, \gamma)$, since the minimum distance is the same for all $t^* \in T^*$.

	Given a surface $\beta(u,v)$ with $u,v \in [0,1]$ and a point $Q \in \mathbb{R}^d$, we can also devise a point inversion algorithm by the same principle. We let $u_i = \frac{i}{n_u}, v_j = \frac{j}{n_v}$ for $i \in \{0, ..., n_u\}, j \in \{0,..., n_v\}$ and $n_u, n_v \in \mathbb{N}$. Then we seek
		$$ m_u, m_v = \underset{(u,v)\in\{u_0, ..., u_n\} \times \{v_0, ..., v_n\}}{\arg\min} \phi(u,v), $$
	where $\phi(u,v) := D(\beta(u,v), Q)$.

	Then we apply Newton's method on $[l_u, u_u] \times [l_v, u_v] = [m_u - \frac{1}{n_u}, m_u + \frac{1}{n_u}] \times [m_v - \frac{1}{n_v}, m_v + \frac{1}{n_v}]$ with the start parameters $m_u, m_v$.

	If the target distance is zero and $\gamma$ is a NURBS curve (or $\beta$ a NURBS surface), we can observe that the devised scheme is the inversion of de Boor's algorithm (Algorithm \ref{alg:debooralgo}), that will return the input parameter that fits the point $Q$. This justifies the name \emph{point inversion algorithm}.
...
\subsection{Intersecting Two Curves in 2D}
	In order to trim curves at intersection points, we require the pair of parameters at which two curves intersect. Let $\gamma_1(s)$ and $\gamma_2(t)$ be two continuous curves in $\mathbb{R}^2$ with $s,t \in [0,1]$ which instersect at finitely many points at most. In this section, we seek for the set of parameters at which these curves intersect.
	...
	\begin{theorem}
		Let $\gamma_1$ and $\gamma_2$ be two continuous curves and let $(s^*, t^*) \in I(\gamma_1, \gamma_2) \cap (0,1)^2$. If the curve derivatives yield $\nabla\gamma_1(s^*) \neq \nabla\gamma_2(t^*)$, then there exist neighborhoods 
	...
	\end{theorem}
	\begin{remark}
 		Suppose that the line segments $\overline{\gamma_1(s_1) \gamma_1(s_2)}$ and $\overline{\gamma_2(t_1) \gamma_2(t_2)}$ intersect. We cannot conclude that there is an intersection $(s,t) \in I(\gamma_1, \gamma_2)$ with $s \in [s_1, s_2]$ and $t \in [t_1, t_2]$. Using this theorem as core concept for our algorithm, we will need to ensure that
 			$$ D(\gamma_1 \domrestr_{[s_1, s_2]}, \gamma_2 \domrestr_{[t_1, t_2]}) \leq \alpha$$
 		for a sufficiently small absolute tolerance $\alpha > 0$. If this is not the case, then no intersection lies inside of $[s_1, s_2] \times [t_1, t_2]$ and every line segment intersection found within should be discarded.
	\end{remark}
...
\subsection{Offset Curves and Self-Intersections in 2D}
	...
	Now, $O^{\gamma}_d \domrestr_{T(O^\gamma_d)}$ is not defined on all of $[0,1]$ if there are any self-intersections in $O^\gamma_d$. However, we can define a domain scaling map $\xi^M$, such that
		$$O^{\gamma}_d(\xi^M([0,1])) = O^{\gamma}_d (M)$$
	for any finite union of closed intervals $M$.
	We write the elements of $M$ as $[s_i, t_i]$ for $i \in \{1,...,m\}$. By setting
...
\subsection{The Creation of Fillets in 2D}
	...
	\begin{remark}
		...
		curve $\phi^{\gamma_1, \gamma_2}_{r^*}$ exists for some radius $r^*$, it does not exist for all radii $r \in \mathbb{R}$.
	\end{remark}
	...
	\begin{theorem}\label{thm:fillettheorem}
		Let $\gamma_1, \gamma_2$ and $r$ such that a fillet circle $\phi^{\gamma_1, \gamma_2}_r$ exists. Then the midpoint $M = (x_0, y_0)$ of a fillet circle is given by
			$$ M = O^{\gamma_1}_r(s_1) = O^{\gamma_2}_r(s_2) $$
		for $(s_1,s_2) \in I(O^{\gamma_1}_r, O^{\gamma_2}_r)$.
	\end{theorem}
	\begin{proof}
		Let $\tilde\phi(t) = r (\sin 2\pi t, \cos 2\pi t) + O^{\gamma_1}_r(s_1)$. Then we can calculate
		\begin{align*}
			\tilde\phi(t) 	&= r (\sin 2\pi t, \cos 2\pi t) + \gamma(s_1) + rN^\gamma(s_1) \\
							&= r (\sin 2\pi t, \cos 2\pi t) + \gamma(s_1) + r\frac{\nabla\gamma(s_1)^\perp}{||\nabla\gamma(s_1)||}.
		\end{align*}
...
\section{Fundamental Terms}
	\begin{itemize}
		\item figure of (part of) turbine
		\item figure of different named sides of turbine blade (ss/ps/le/te)
	\end{itemize}
	...
	We also make the distinction between two kinds of blades, rotor and stator blades. While the rotor blades rotate around the hub, the stator blades have fixed positions. This was taken into account when designing CoolingGen, so both kinds are currently supported.

	The blade surface $B(u,v)$ has a named partitioning in $u$-direction into four distinct parts. The leading edge, the suction side, the trailing edge and the pressure side.

\section{The S2 Stream Surface and Stream Surface Coordinates}
	\begin{itemize}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=.5\textwidth]{../assets/wu_naca_s2.png}
		\caption{Example $S_1$ and $S_2$ surfaces of a turbomachine. The $z$ coordinate in the figure is $x$.}
	\end{figure}
...
\section{Chambers}
	missing figures: 
	\begin{itemize}
		\item first figure, first subfig: labeled figure with all curve names and $p_i^+, p_i^-$ 
		\item in second subfig: single chamber without any offset
		\item second figure: offsetting curves, then intersecting curves, then adding fillets as 3 subfigs
		\item offset and trimming in single steps always
	\end{itemize}
	Channels are hollow sections inside a turbine blade which allow for the coolant to pass through. For the construction of channels, we distinguish between straight and curved sections of the channel. The straight sections represent hollow bodies that let the coolant pass in $v$-direction of the turbine blade. These sections are called \emph{chambers}. The curved sections let the coolant pass in $u$-direction of the turbine blade. We cal the latter sections \emph{turns}. Inside these turns, the coolant flows between chambers. In this section, we present how chambers are constructed, whereas turn construction is throughly explained in the next section.

	\begin{figure}[H]
		\centering
		\begin{subfigure}{.4\textwidth}
			\includegraphics[width=\textwidth]{../tec/shrinking/offset.png}
		\end{subfigure}
		\begin{subfigure}{.4\textwidth}
			\includegraphics[width=\textwidth]{../tec/shrinking/trimming.png}
		\end{subfigure}
		\caption{Offsetting and trimming a $(m, r\theta)$ profile, then adding fillets where necessary.}
	\end{figure}

	First, we apply the stream surface coordinate transformation to a number of $v$-isoparametric curves of the blade to calculate a fixed number of profiles $P^{(v_i)}_\textrm{stream}(u)$ for $i \in \{1,...,N\}$. These profiles are then offset by an input parameter. The profile is automatically trimmed adequately and fillets are added, the radius of which are declared once again as an input. We refer to the resulting profile as \emph{shrunk profile} and declare it $S^{(v_i)}(t)$.

	\begin{figure}[H]
		\centering
		\begin{subfigure}{.4\textwidth}
			\includegraphics[width=\textwidth]{../tec/chambers/walls.png}
			\caption{Subdivision of profile along camber.}
		\end{subfigure}
		\phantom{aa}
		\begin{subfigure}{.5\textwidth}
			\includegraphics[width=\textwidth]{../tec/chambers/single_constr.png}
			\caption{Offsetting, trimming and adding fillets.}
		\end{subfigure}
		\caption{Construction of a chamber profile.}
	\end{figure}

	We calculate the camber curve of each shrunk profile $S^{(v_i)}$. The camber curve $c^{(v_i)}(t)$ is a design aid which is defined by the following property. A point $x$ lies on the camber curve $c^{(v_i)}$ of $S^{(v_i)}$ if and only if for some $d$ its offset curve $O^{S^{v_i}}_d$ has a self intersection at $x$. Along the camber curve, we define the wall input, which is given by a sorted set of parameters $w_i$ representing the positions of each wall relative to the camber curve, $i \in \{1,...,N_C-1\}$, where $N_C$ is the number of chambers that shall be created. For each position, we also define an angle $\omega_i$. Each position-angle pair represents two rays
		$$ c(w_i) + sR_{\omega_i} \nabla c(w_i) \quad\text{and}\quad c(w_i) - sR_{\omega_i} \nabla c(w_i) $$
	for $s \in [0, \infty)$, where $R$ is a rotation matrix like the one presented in ???. Using the ray marching algorithm, we can find the intersection $p_i^+$ and $p_i^-$ of the rays and the profiles. The line segment $\overline{p_i^+p_i^-}$ now represents the $i$-th wall. 

	The blade is then partitioned at these intersection points. For each such partitioning, we are left with four curves per chamber profile. Some of these curves are the walls, which are curves that represent straight lines between $p_i^+$ and $p_i^-$. There are four distinct cases. The first case is $N_C = 1$. In this case, the four curves are given by the pressure side, the leading edge, the suction side and the trailing edge partitioning of the input blade surface $B(u,v)$. If however $N_C > 1$, we have three more cases. The chamber profile located at the leading edge, which we will refer to as leading chamber profile, consists of the partition of the pressure side between the leading edge and $p_1^+$, the leading edge, the partition of the suction side between the leading edge and $p_1^-$ and lastly the first wall curve. The chamber profile located at the trailing edge, which we will refer to as trailing chamber, consists of the partition of the pressure side between trailing edge and $p_{N_C-1}^+$, the $(N_C-1)$-th wall, the partition of the suction side between trailing edge and $p_{N_C-1}^-$ and the trailing edge. For the $i$-th chamber profile, where $i \neq 1$ and $i \neq N_C$, these four curves are the partition of the pressure side between $p_i^+$ and $p_{i+1}^+$, the $i$-th wall, the partition of the suction side between $p_{i}^-$ and $p_{i+1}^-$ and the $(i+1)$-th wall.

	After performing this partitioning, we can offset the curves. However, each curve is offset differently. In the wall input, the thickness of the wall in each direction is specified as an input. In fact, this input specification is done using NURBS curves. The user of CoolingGen specifies control points $(v_k, d_k)$, where $v_k \in [0,1]$ is the surface parameter and $d_k$ is the respective thickness with $k \in [1, ..., K]$. The leading and trailing profile offsets are linearly interpolated between suction and pressure side offsets to guarantee differentiability of the chamber profile at the leading and trailing edge transitions, respectively. Since after offsetting, the four chamber profile parts do not necessarily align, we connect the curve ends either by intersection, ray-curve intersection or ray-ray intersection, depending on which one is applicable. The rays are in this case defined by $R_{A,B}$, where $A$ represent the curve boundary points and $B$ represent the gradient at the curve boundary points.

	After connecting and trimming the offset curves using the methods in section ???, we are left with four trimmed offset curves that non-differentiably connect offset wall curves. We use the fillet curve method presented in ?? to make their connection a circular arc.

	We can use the inverse coordinate transformation from the $(m, r\theta)$ stream surface coordinate system to the Cartesian coordinate system. Interpolation between the transformed chamber profiles yields $N_C$ three-dimensional chamber surfaces.
