...
\subsection{Ray-Curve Intersection}
	...
	If however $I = \emptyset$, then no element will be returned. We therefore define the return value of our algorithm as the one element set given by
		$$ I_{\min} := 
			\begin{cases}
				\{R_{A,B}(s^*)\} 	&\text{if } D(R_{A,B}(s^*), \gamma) = 0, \\
	...
	\begin{algorithm}[H]
		\begin{algorithmic}[1]
			...
			\Procedure{intersectCurveRay}{$\gamma, A, B$}
				\State $B \gets \frac{B}{||B||}$
				\State $s \gets 0$
				\State $P \gets A$
				\For {$k = 1, 2,..., N$}
					\State $s \gets s + \text{pointInversionDistance}(\gamma, P)$
					\State $P \gets A + sB$
					\If {$s < \epsilon$}
						\Return $\{P\}$
					\EndIf
	...
	In this algorithm, $\text{pointInversionDistance}(\gamma, P)$ calculates the $D(\gamma, P)$ as was discussed in the previous section.
