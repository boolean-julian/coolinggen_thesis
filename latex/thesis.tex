...
To efficiently calculate points on a NURBS curve, Carl-Wilhelm Reinhold de Boor devised an algorithm.
	...
	\begin{tikzpicture}
		...
		\foreach \x in {1,...,\degree}{
			\foreach \y in {1,...,\x}{
				...
				\ifthenelse{\y > 1} {
					\draw [<-] (\arabic{z}) -- (\arabic{a}) node[above, midway, sloped] {\scriptsize $1-\alpha^{(\pgfmathint{\degree-\x+1}\pgfmathresult)}_{l-\pgfmathint{\y-1}\pgfmathresult}$};
					\draw [<-] (\arabic{z}) -- (\arabic{b}) node[above, midway, sloped] {\scriptsize $\alpha^{(\pgfmathint{\degree-\x+1}\pgfmathresult)}_{l-\pgfmathint{\y-1}\pgfmathresult}$};
				}
				{
					\draw [<-] (\arabic{z}) -- (\arabic{a}) node[above, midway, sloped] {\scriptsize $1-\alpha^{(\pgfmathint{\degree-\x+1}\pgfmathresult)}_l$};
					\draw [<-] (\arabic{z}) -- (\arabic{b}) node[above, midway, sloped] {\scriptsize $\alpha^{(\pgfmathint{\degree-\x+1}\pgfmathresult)}_l$};
				}
