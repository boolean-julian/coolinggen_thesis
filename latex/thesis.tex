...
\subsection{Point Inversion}
	...
	where $\phi(u,v) := D(\beta(u,v), Q)$. Then we apply Newton's method on $[l_u, u_u] \times [l_v, u_v] = [m_u - \frac{1}{n_u}, m_u + \frac{1}{n_u}] \times [m_v - \frac{1}{n_v}, m_v + \frac{1}{n_v}]$ with the start parameters $m_u, m_v$.
...
\section{Chambers}
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{Shrinking a profile in $(m, r\theta)$. The black curve represents $P^{(v_i)}_\textrm{stream}(u)$, whereas the red curve in (c) represents $S^{(v_i)}_\textrm{stream}(u)$.}
		\label{fig:profileshrinking}
	\end{figure}

	First, we apply the stream surface coordinate transformation presented in Section \ref{sec:mrtheta} to a fixed number $N$ of $v$-isoparametric curves of the blade to calculate the profiles $P^{(v_i)}_\textrm{stream}(u)$ for $i \in \{1,...,N\}$. These profiles are then offset by an input parameter. The profile is automatically trimmed adequately and fillets are added, the radius of which are also declared as an input. We refer to the resulting profile as \emph{shrunk profile} and declare it $S^{(v_i)}_\textrm{stream}(u)$. This process is depicted in Figure \ref{fig:profileshrinking}.
