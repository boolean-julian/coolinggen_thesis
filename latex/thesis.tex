...
\abstract{Abstract}{
	The generation of cooling geometries within turbine blades is an important aspect of jet engine design and research. Generating such geometries with conventional CAD tools is very time-consuming. CoolingGen is a fully parametric CAD tool for the fast creation of cooling geometries within turbine blades. Due to its parametrization, there is a semantic interface between user input and mechanical function, which is not given in conventional CAD tools. In this thesis, the theoretical and practical aspects of parametric geometry generation by CoolingGen are highlighted. It serves as a manual of important geometric methods and processes within CoolingGen.
}
\vspace{2cm}

\abstract{Zusammenfassung}{
	Die Generierung von Kühlungsgeometrien innerhalb von Turbinenschaufeln ist ein wichtiger Teilaspekt des Triebwerkdesigns und der Triebwerksforschung. Mit konventionellen CAD Tools lassen sich solche Geometrien zwar erzeugen, allerdings nur unter großem Zeitaufwand. CoolingGen ist ein gänzlich parametrisches CAD Tool zur schnellen Erstellung von Kühlungsgeometrien innerhalb von Turbinenschaufeln. Durch die Parametrisierung gibt es eine semantische Schnittstelle zwischen Eingabe und mechanischer Funktion, welche in konventionellen CAD Tools nicht gegeben ist. In dieser Thesis werden die theoretischen und praktischen Aspekte der parametrischen Geometrieerzeugung durch CoolingGen beleuchtet. Sie dient als Handbuch wichtiger geometrischer Methoden und Prozesse innerhalb von CoolingGen.
}
\pagebreak
