...
\subsection{Properties}
	...
	\begin{theorem}
		Let $V = \mathbb{R}^d$ and $T \in \mathbb{R}^{d \times d}$. Let $S_P$ be a B-spline curve or surface and $C_P$ a NURBS curve or surface. Then $S_{TP} = TS_P$ and $C_{TP} = TC_P$.
	\end{theorem}
	\begin{proof}
...
\subsection{Ray-Curve Intersection}
	\begin{definition}
		A \emph{ray} with support vector $A \in V$ and direction vector $B \in V$ is given by the set of points
