...
\subsection{De Boor's Algorithm}
	...
	The algorithm is visually represented in Figure \ref{fig:debooralgo}. To calculate one point on the B-spline curve $S_P$ of degree $p$, we require at most $\frac{p^2-p}{2}$ vector additions and $p^2-p$ scalar multiplications, similarly to what we found for Bézier curves. However, we also need to calculate $\omega_{j,k}$ at every step, which in total sums up to $p^2-p$ real additions and $\frac{p^2-p}{2}$ real multiplications.
	...
	\begin{algorithm}[H]
		\begin{algorithmic}[1]
			...
			\Procedure{intersectCurvesRecursion}{$I, \gamma_1, l_1, u_1, \gamma_2, l_2, u_2, d$}
				...
				\If {$D(\gamma_1(m_1), \gamma_2(m_2)) \leq \alpha$} append $(m_1, m_2)$ to $I$ and \Return $I$
				\EndIf

				\If {$d \geq d_{\max}$} do nothing and \Return $I$
				\EndIf
...
\section{Ejection Slots}\label{sec:slots}
	...
	\begin{figure}[H]
		...
		\caption{Connecting $I(u,v)$ and $O(u,v)$ (orange) using surface patches (blue).}
