...
\subsection{Intersecting a Surface and a Plane}
	...
	\begin{figure}[H]
		...
		\caption{In our case, intersecting a surface (blue) and a plane (grey) yields a curve (orange).}
	...
	Using an appropriate step size $\alpha \in \mathbb{R}$, we can project the point $L^{(0)}(\alpha)$ (which lies in both the tangent plane $T_{\beta}(u^{(0)}, v^{(0)})$ and the target plane $P_{S,N} = P_{\beta(u^{(0)}, v^{(0)}), N}$) onto the surface $\beta$ using the methods from Section \ref{subsec:pointproj} to acquire a candidate for the point $\beta(u^{(1)}, v^{(1)})$. If this point lies outside of the plane $P_{S,N}$, we discard the point and choose a step size less than $\alpha$ and repeat the process. If such a point $\beta(u^{(1)}, v^{(1)})$ exists, we can repeat the process to search $\beta(u^{(2)}, v^{(2)})$. To do this, we construct $L^{(1)}$ by intersecting $P_{S,N}$ and $T_{\beta}(u^{(1)}, v^{(1)})$ and project $L^{(1)}(\alpha)$ onto $\beta$. We can repeat this process for some $alpha_0 > 0$ until $|\alpha| < \alpha_0$. If this condition is satisfied, we return the list of $n$ parameters $L = \{(u^{(i)}, v^{(i)}): i \in \{1,2,...,n\}\}$.
