...
\subsection{Intersecting a Surface and a Plane}
	In this section, we want to find an algorithm that intersects a plane and a surface in $\mathbb{R}^3$. The intersection genereally takes the form of a surface, a curve, a point or any union of the aforementioned. In the case of CoolingGen, we only care about the case that the intersection is a single curve. This assumption is implicitly made in many of the statements involving the intersection algorithm. First, we need to introduce some basic notions.
	...
	\begin{figure}[H]
		...
		\caption{Intersecting a surface (blue) and a plane (grey) yields a curve (orange).}
		\label{fig:planeintersectionexample}
	\end{figure}

	\begin{definition}
		The distance between a plane $P_{S,N}$ and a point $Q \in \mathbb{R}^3$ is defined as
			$$ D(P_{S,N}, Q) := \underset{V \in P_{S,N}}{\arg\min} \; D(V, Q). $$
	\end{definition}

	\begin{theorem}
		The expression $\langle (Q - S), N \rangle$ solves $\underset{V \in P_{S,N}}{\arg\min} \; D(V, Q).$
	\end{theorem}
	...
	Using an appropriate step size $\alpha \in \mathbb{R}$, we can project the point $L^{(0)}(\alpha)$ (which lies in both the tangent plane $T_{\beta}(u^{(0)}, v^{(0)})$ and the target plane $P_{S,N} = P_{\beta(u^{(0)}, v^{(0)}), N}$) onto the surface $\beta$ using the methods from Section \ref{subsec:pointproj} to acquire the point $\beta(u^{(1)}, v^{(1)})$. If this point lies too far away from the tangent intersection line $L^{(0)}$, we discard the point and choose a step size less than $\alpha$ and repeat the process. This iteration step is repeated until the step size falls below a certain threshold $\alpha_0$.
	...
	\begin{algorithmic}[1]
		...
		\EndInput
		\Output
			\Desc{$\{u^{(i)}, v^{(i)}) : i \in \{1, 2, ...\}\}$}{set of points on the intersection}
		\EndOutput
