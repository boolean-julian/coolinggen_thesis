...
\subsection{Curve-Curve Intersection in 2D}
	...
	If for any intersection parameter pair $(s^*, t^*) \in I(\gamma_1, \gamma_2)$ we have $\nabla \gamma_1(s^*) = \nabla \gamma_2(t^*)$, then it is very likely that the intersection is not detected by the described line segment intersection scheme. In CoolingGen, however, this intersection scheme performs well.
	...
	\begin{algorithm}[H]
		\begin{algorithmic}[1]
			...
			\Procedure{intersectCurvesRecursion}{$I, \gamma_1, l_1, u_1, \gamma_2, l_2, u_2, d$}
				...
				\State $P^{(1)} \gets \text{getPiecewiseLinearInterpolation}(\gamma_1, l_1, u_1, N)$
				\State $P^{(2)} \gets \text{getPiecewiseLinearInterpolation}(\gamma_2, l_2, u_2, N)$
