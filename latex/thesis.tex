...
\section{Fundamental Terms}
	\begin{itemize}
		...
		\item explain where exactly we are in the turbine
		\item stator/rotor difference
		\item show distribution of (u,v) on blade
...
\section{The S2 Stream Surface and Stream Surface Coordinates}
	...
	where $x$ is the axial coordinate, $r = \sqrt{y^2+z^2}$ the distance from the hub and $m$ is the arc length.

	To design the blades for turbomachinery, BladeGen and CoolingGen make use of a certain $S_2$ surface, the so called $S_{2,m}$ surface. This surface lies midway between two blades. Each stream line described by $S_{2,m}$ can then be written as $S_{2,m}(\cdot, v)$. By intersecting the $u$-isoparametric curves $B(u,\cdot)$ of the blade $B(u,v)$ with each stream line, we can construct a planar curve $P^{(v)}_\textrm{stream}(u)$ from the intersection points $P_{u,v}$, given by
		$$ P^{(v)}_\textrm{stream}(u) = (m, r\theta),$$
	where $m$ is the intersection arc length, $r$ is the distance from hub of intersection and the circumferential angle $\theta$ is given by
		$$ \theta = \arctantwo(y, z). $$
	This coordinate system with respect to arc length $m$ and radius-angle product $r\theta$ is a stream surface coordinate system. One should note that for this coordinate transformation, $B(u,v)$ has to be transformed to cylindrical coordinates $(x,r)$ before intersection with the stream lines. The stream surface coordinate transformation is invertible given $v$ and $S_{2,m}$. Even though the input blade surface $B(u,v)$ lies in $\mathbb{R}^3$, the most important methods of CoolingGen (and BladeGen) are operations on objects in $(m, r\theta)$.

\chapter{Results}

\section{Chambers}
	\begin{itemize}
		\item what are chambers
		\item what are they good for
		\item how do we construct them
	\end{itemize}
	Channels are hollow sections inside a turbine blade which allow for the coolant to pass through. For the construction of channels, we distinguish between straight and curved sections of the channel. The straight sections represent hollow bodies that let the coolant pass in $v$-direction of the turbine blade. These sections are called \emph{chambers}. The curved sections let the coolant pass in $u$-direction of the turbine blade between chambers. We cal the latter sections \emph{turns}. In this section, we present how chambers are constructed.
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{Offsetting and trimming a profile, then adding fillets where necessary.}
	\end{figure}

	First, we apply the $(m, r\theta)$ coordinate transformation to a number of $v$-isoparametric curves of the blade to calculate the profile $P^{(v_i)}_\textrm{stream}(u)$ for $i \in \{0,...,N\}$ as described in ???. These profiles are then offset by an input parameter. The profile is trimmed adequately and fillets are added, the radius of which are declared once again as an input.
	...
	We calculate the camber curve of the shrunk profile. The camber curve $c(t)$ with $t \in [0,1]$ is a design aid which is defined by the following property. A point $x$ lies on the camber curve $c$ if and only if for some $d$ its offset curve $O^c_d$ has a self intersection at $x$. Along the camber curve, we define the wall input, which is given by a set of parameters $t_i$ representing the positions of each wall relative to the camber curve, $i \in \{1,...,N_C-1\}$, where $N_C$ is the number of chambers that shall be created. For each position, we also define an angle $\phi_i$. Each wall position and angle pair represents two rays
		$$ c(t_i) + sR_{\phi_i} \nabla c(t_i) \quad\text{and}\quad c(t_i) - sR_{\phi_i} \nabla c(t_i) $$
	for $s \in [0, \infty)$. Using the ray marching algorithm, we can find the intersection $p^+$ and $p^-$ of the rays and the profiles. The line segment $\overline{p^+p^-}$ now represents the wall between two chambers. 
