...
\chapter{Introduction}
	Modelling cooling geometries inside of existing turbine blades and vanes using conventional interactive computer-aided design (CAD) methods currently is a difficult but necessary process in the design of modern jet engine turbines. For this purpose, the German Aersospace Center (DLR) has been concerned with finding a simple method to model such structures. As part of many CAD-related research projects at the DLR, parametric CAD has evidently been a simpler alternative to its conventional counterpart. The complexity and inevitable inter-dependence of cooling structures has ever since been a challenge in the development of a parametric CAD tool made solely for turbine cooling. In this thesis, we present a high-performance tool, CoolingGen, that uses custom-made algorithms and evidently satisfies the goal of creating models of such cooling structures.
	...
	\section{Cooling Design}
		\begin{figure}[H]
			\centering
			\includegraphics[width=.75\textwidth]{../assets/gthandbook_turbine.png}
			\caption{Jet engine high pressure turbine and combustor as depicted in \cite{Giampaolo2020}.}
		\end{figure}
	...
	\section{State of the Art}
		Conventional CAD systems are modelers that allow their user to create free-form curves and surfaces using interactive control points that can be moved to any location on a three-dimensional canvas. This gives the user an unlimited amount of strategies for creating a CAD model fit for their use. However, the amount of possible modelling strategies impedes the adaptability of most resulting models. An alternative to conventional CAD systems are feature-based parametric CAD systems, which relies on a parent-child structure of a mechanical model and its constituent parts (see \cite{Shah1995} and \cite{Camba2016}). This behavior can also be observed in the parametric CAD software GTLab, which is a tool developed at the DLR that combines different aeroplane geometries in an associative manner. In parametric CAD software, the user specifies a select set of (non-)geometric parameters that determine the shape of the output geometry. In mechanical engineering, feature-based CAD has become an important and widely used tool.

		In parametric CAD, the number of input parameters determines the flexibility over the shape of a model. A high number generally improves flexibility. Although keeping this number as low as possible ensures simple user interaction. As Albert Einstein once famously said, \emph{everything should be made as simple as possible, but not simpler}. To adapt the geometry, the user simply has to change a few parameters instead of moving control points, which in itself bear no underlying semantic structure regarding their mechanical function.
 		
		Creating cooling geometries is laborious and requires expertise using conventional interactive CAD software. The approach of automating this process using a set of parameters certainly bears great potential in this regard. At the DLR, blade and vane contours are already successfully designed using parametric CAD in a tool called BladeGen. In the associative structure of parametric CAD, blades and vanes serve as the parent element of the cooling geometries we are concerned with in the development of CoolingGen.

	\section{Problem Statement}
		CoolingGen is a parametric CAD software written in C that allows for the creation of three-dimensional turbine cooling geometries of blades and vanes. It was first developed by Timo Schumacher and Christian Voß in 2013 at the DLR. In 2022, it was in large part revised to support realistic cooling geometries. The 2013 release, while being a great proof of concept, lacked an underlying framework of established geometric methods and thus failed to fulfill real world requirements. It was tailored to fit only a few parameter configurations on a certain parent blade surface. The implementation of such established geometric methods was done during the course this thesis.
	...
	\section{Turbine Design Process Chain}
		Missing :-(
...
\chapter{Discussion}
	% Discuss results: Say what's good
	% 	High performance: Machine specs + time for a setup (with setup description)
	% 	CENTAUR/tec
	%	Can add new cooling geometries -> Future work
	%
	% Current shortcomings
	%	Everything as single surface instead of one merged body
	%
	% Future Work
	% 	Make full body
	%	Make more geometries: Pin fin cooling, other film cooling delimiting curves
	%	Embedding in GTLab
