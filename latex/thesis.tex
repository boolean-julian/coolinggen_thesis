...
\subsection{Curve-Curve Intersection in 2D}
	...
	\begin{theorem}
		If $L_A$ and $L_B$ are not parallel, the single intersection point of the lines that contain $L_A$ and $L_B$ is given by $L_A(t_A) = L_B(t_B)$, where
	...
	\begin{algorithm}[H]
		\begin{algorithmic}[1]
			...
			\Procedure{getPiecewiseLinearInterpolation}{$\gamma, l, u, N$}
			...
			\Procedure{intersectCurvesRecursion}{$I, \gamma_1, l_1, u_1, \gamma_2, l_2, u_2, d$}
				\State $m_1 \gets \frac{l_1 + u_1}{2}$
				\State $m_2 \gets \frac{l_2 + u_2}{2}$
				\If {$D(\gamma_1(m_1), \gamma_2(m_2)) \leq \epsilon$}
					\State $\text{append } (m_1, m_2) \text{ to } I$
					\State \Return
				\EndIf
				...
				\State $P^{(1)} \gets \text{getPiecewiseLinearInterpolation}(\gamma_1, 0, 1, N)$
				\State $P^{(2)} \gets \text{getPiecewiseLinearInterpolation}(\gamma_2, 0, 1, N)$

				\For {$i = 0,...,N-1$}
					\For {$j = 0,...,N-1$}
						\If {$\text{lineSegmentsIntersect}(P^{(1)}_i, P^{(1)}_{i+1}, P^{(2)}_j, P^{(2)}_{j+1})$}
							\State $s_1 \gets (l_1 - u_1) \frac{i}{N} + l_1$
							\State $s_2 \gets (l_1 - u_1) \frac{i+1}{N} + l_1$
							\State $t_1 \gets (l_2 - u_2) \frac{j}{N} + l_2$
							\State $t_2 \gets (l_2 - u_2) \frac{j+1}{N} + l_2$
