...
\section{Fundamental Terms}
	The creation of cooling geometries inside of turbine blades is vital for the enhancement of the thermodynamic efficiency and longevity of turbine engines. In modern turbines, the materials thermal limits are exceeded by the hot gas. To withstand this extreme condition, cooling is required. The blade geometries that we are concerned with in the development of CoolingGen are the ones that succeed the inlet in axial direction.
...
\section{The S2 Stream Surface and Stream Surface Coordinates}\label{sec:mrtheta}
	...
	is the $v$-isoparametric arc length of the stream surface. To design the blades for turbomachinery, BladeGen and CoolingGen make use of a certain $S_2$ surface, the so called $S_{2,m}$ surface. This surface lies midway between two blades \cite{Wu1952}. Each stream line described by $S_{2,m}$ can then be written as $v$-isoparametric curve $S_{2,m}(\cdot, v)$.
	...
	Since this coordinate transformation produces a set of planar curves for $v_i \in [0,1]$, where $i \in \{1, 2, ..., N\}$, we can use two-dimensional operations on such a set of stream surface curves instead of using three-dimensional operations directly on the Cartesian represented $B(u,v)$. This yields a significant performance boost. The resulting stream surface curves can then be transformed back to the Cartesian coordinate system. Using surface skinning we can interpolate between families of stream surface curves to find a surface (see \cite{Piegl1997}). This approach will be used in the construction of chambers and impingement inserts.

\chapter{Results}
	\begin{figure}[H]
		\centering
		\includegraphics[width=.8\textwidth]{../tec/complete/11.png}
		\caption{Rotor and stator blade with cooling geometries.}
	\end{figure}
...
\section{Chambers}\label{sec:chambers}
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{Profile partitioning along rays that emerge from the camber curve. The camber curve is represented by the dashed line.}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{0.49\textwidth}
			\centering
			\includesvg[width=0.5\textwidth]{../python/linearContinuationCurve}
			\caption{Linear continuation of two curves.}
		\end{subfigure}
		\begin{subfigure}{0.49\textwidth}
			\centering
			\includesvg[width=0.5\textwidth]{../python/linearContinuationCurve2}
			\caption{Intersection of the linear continuation.}
		\end{subfigure}
		\caption{Ensuring intersection by linear continuation.}
...
\section{Film Cooling Holes}\label{sec:filmcooling}
	...
	As mentioned before, film cooling holes transport the fluid between the inside of a channel and the outside of the blade. Therefore it is necessary to connect $F^{(v_\textrm{transition})}(u,v)$ with the blade surface and the corresponding channel surface. Our aim is to align the $f_1$ side of $F^{(v_\textrm{transition})}$ to the blade surface and the $f_2$ side of $F^{(v_\textrm{transition})}$ to one of the channel surfaces.
	...
	We are left with three important curves. The blade delimiting curve $B_d(u)$, the transition isocurve $F^{(v_\textrm{transition})}(u,v_\textrm{transition})$ and the channel delimiting curve $C_d(u)$. We can linearly interpolate between these three curves. The intersected film cooling hole surface is then equal to
		$$ \Phi(u,v) = 
				\begin{cases}
...
\section{Impingement Inserts}
	...
	The profile partitions that run parallel to the blade are offset by $o$, whereas the profile partitions that lie between chambers are offset by $i$. The new profile partitions are trimmed and fillets are added. This yields the outer impingement insert profile. The outer impingement insert profile is then offset by $d$ to find the inner impingement insert profile. After using surface skinning, we are left with two surfaces that represent the impingement insert, an example of which can be seen in Figure \ref{fig:impingementinsert}.
...
\section{Slots}
	Slots transport the coolant from the inside of the leading edge channel to the pressure side of the blade. A slot is partially defined by two delimiting surfaces, the inlet surface $I(u,v)$ and the outlet surface $O(u,v)$ for $u,v \in [0,1]$. The inlet surface $I(u,v)$ is a subsurface of the channel surface $C(u,v)$ that lies closest to the trailing edge. The outlet surface $O(u,v)$ is a subsurface of the blade surface $B(u,v)$.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.3\textwidth]{../tec/slots/16.png}
	...
	The boundaries of the emerging surface, which are represented in red in Figure \ref{fig:slot_connection}, can be connected by a special surface called Coons patch (reference to Coons). Combining the Coons patches with the suction- and pressure-sided surfaces we created, we can make a slot surface that is closed (see left side of Figure \ref{fig:slot_coon}).

	Next, we add fillets to the isoparameter curves of the slot curves. Since the isoparametric curves are not planar, we cannot use the methods from Section \ref{sec:filletcurves}. Instead, we use a different approach using a different assumption. We assume that the corners of the isoparameter curves are right-angled corners. In this case, we can use the fillet procedure shown in Figure \ref{fig:slot_coon}. To do this, we trim the curves near the corner by the input radius $r$ and connect the trimmed curves using a circular arc.
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{An array of slots with realistic values of $\omega_L$ and $\omega_H$.}
