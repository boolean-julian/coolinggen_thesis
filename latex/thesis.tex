...
\subsection{Offset Curves \& Self-Intersections in 2D}\label{sec:offsetcurves}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{0.3\textwidth}
			...
			\caption{}
		\end{subfigure}
		\caption{In (a), the blue curve $\gamma_1$ and the blue curve $\gamma_2$ are offset by different amounts $d_1$ and $d_2$, respectively. Offsetting the orange curve $\beta$ by a constant offset results in a curve that does not align with $\gamma_1$ and $\gamma_2$ simultaneously. However, if we offset $\beta$ by a function $f(t)$ as in (b) instead, we can smoothly connect $\gamma_1$ and $\gamma_2$ using the cubic interpolation offset curve $O^{\beta}_{f(t)}(t)$, which can be seen in (c).}
