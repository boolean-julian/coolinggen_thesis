...
\chapter{Introduction}
	The creation of cooling geometries inside of turbine blades is vital for the enhancement of the thermodynamic efficiency and longevity of turbine engines. In modern jet engine turbines, the materials thermal limits are exceeded by the hot gas temperature. To withstand this extreme condition, cooling is required. The blade geometries that we are concerned with in the development of CoolingGen are the ones that succeed the inlet in axial direction.
...
\chapter{Jet Engine Specific Methods}\label{chap:jetenginespecifics}
	...
	A distinction is made between two kinds of blades, rotor blades and stator vanes. While the rotor blades rotate around the hub, the stator vanes have fixed positions. Furthermore, the blade surface $B(u,v)$ has a named partitioning in $u$-direction into four distinct parts. The leading edge, the suction side, the trailing edge and the pressure side.
...
\chapter{Results}
	...
	Channels are hollow sections inside a turbine blade which allow for a coolant to pass through.
...
\subsection{Chambers}\label{sec:chambers}
	...
	First, we apply the stream surface coordinate transformation presented in Section \ref{sec:mrtheta} to a fixed number $N$ of $v$-isoparametric curves of the blade to calculate the profiles $B^{(v_i)}_\textrm{stream}(u)$ for $v_i \in [0,1]$, $i \in \{1,...,N\}$. These profiles are then offset by an input parameter. The profile is automatically trimmed adequately (as described in Section \ref{sec:offsetcurves}) and fillets are added to round off the emerging corners. The fillet radius is also declared as an input. We refer to the resulting profile as shrunk profile $S^{(v_i)}_\textrm{stream}(u)$. This process is depicted in Figure \ref{fig:profileshrinking}. The subdivision into leading edge, pressure side, suction side and trailing edge is kept by using the fillet boundaries as transition points.
...
\section{Film Cooling Holes}\label{sec:filmcooling}
	...
	As mentioned before, film cooling holes transport the fluid from the inside of a channel to the outside of the blade. Therefore it is necessary to connect $F^{(v_\textrm{transition})}(u,v)$ with the blade surface and the corresponding channel surface. Our aim is to align the $f_1$ side of $F^{(v_\textrm{transition})}$ to the blade surface and the $f_2$ side of $F^{(v_\textrm{transition})}$ to one of the channel surfaces.
