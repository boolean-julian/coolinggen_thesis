...
\subsection{Curve-Curve Intersection in 2D}
	...
	\begin{figure}[H]
		...
		\centering
		\includesvg[width=0.75\textwidth]{../python/curveIntersectionConnectedness}
		\caption{In the top row, we have $|I(\gamma_1, \gamma_2)| < \infty$, which is the case considered in this thesis. In the bottom row, $|I(\gamma_1, \gamma_2)| = \infty$. In the left column we can see the intersection points, whereas the right column represent the set $I(\gamma_1, \gamma_2)$.}
	\end{figure}
...
\subsection{Fillet Creation}
	Given two curves $\gamma_1$ and $\gamma_2$ defined on $t \in [0,1]$ that intersect at their end points, we can construct a circle of a given radius that shares one tangential intersection point with each curve. A circle like this is called a fillet circle. More formally, we can make the following definitions

	\begin{definition}
		Let $\gamma_1$ and $\gamma_2$ be two differentiable curves.
		If curve $\phi^{\gamma_1, \gamma_2}_r$ satisfies
	...
	If it exists, we will find a procedure that yields a fillet circle for a given radius $r$ and a pair of curves $\gamma_1, \gamma_2$. We only need to find the center point $(x_0, y_0)$. Then we can use the parametric definition of a circle to write
		$$ \phi^{\gamma_1, \gamma_2}_r(t) = (r \cos(2 \pi t) + x_0, r \sin(2 \pi t) + y_0).$$
	For example, as shown in Figure \ref{fig:verysimplefillet}, given the curves $\gamma_1(t) = (2t-1,1)$ and $\gamma_2 = (1,2t-1)$ and a radius $r = 1$, the center point $(x_0,y_0)$ of the fillet circle is equal to $(0,0)$.




	We can use offset curve and intersection methods to find the center point $(x_0, y_0)$ in the general case.
